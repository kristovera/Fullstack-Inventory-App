...
const reqValidationNonFieldOwnership = async (res) => {
...
const reqValidationCreate = validationMiddleware([
  textField("no_req"),
  itemsField("products"),
  objectField("depts"),
  ...
  numberField("products.*.qty").custom(async (qty, { req, path }) => {
    const index = Number(path.replace(/\D/g, ""));
    const _id = req.body.products[index]._id;
    const item = await Item.findOne({ _id });
...
module.exports = {
  reqValidationCreate,
  reqValidationNonFieldOwnership,
};
